...
public class XposedHook implements IXposedHookLoadPackage, IXposedHookZygoteInit {
	...
	private static final Class<?>[] HAPPY_CLASS = new Class[] { AbsListView.class, WebView.class, ScrollView.class,
		...
		findAndHookMethod(StatusBarWindowView, "onInterceptTouchEvent", MotionEvent.class, new XC_MethodHook() {
			...
			protected void afterHookedMethod(MethodHookParam param) throws Throwable {
				...
				case MotionEvent.ACTION_MOVE:
					if (mIsClick
							&& (Math.abs(mDownX - ev.getX()) > SCROLL_THRESHOLD || Math.abs(mDownY - ev.getY()) > SCROLL_THRESHOLD)) {
						mIsClick = false;
					}
					break;
				default:
